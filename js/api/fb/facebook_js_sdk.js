...
FB.provide('', {
  ...
  ui: function(params, cb) {
    ...
    if (FB._nativeInterface) {
        var alertTitle = 'Algo salió mal\n\n';
        switch (params.method) {
            case 'auth.login':
                FB._nativeInterface.login(params, cb, function(e) {ToastL(alertTitle + e);});
                break;
            ...
            case 'permissions.oauth':
                ...
                break;
            case 'auth.logout':
                FB._nativeInterface.logout(cb, function(e) {ToastL(alertTitle + e);});
                break;
            case 'auth.status':
                FB._nativeInterface.getLoginStatus(cb, function(e) {ToastL(alertTitle + e);});
                break;
            case 'login.status':
                FB._nativeInterface.getLoginStatus(cb, function(e) {ToastL(alertTitle + e);});
                break;
            case 'feed':
                FB._nativeInterface.dialog(params, cb, function(e) {ToastL(alertTitle + e);});
                break;
            case 'apprequests':
                FB._nativeInterface.dialog(params, cb, function(e) {ToastL(alertTitle + e);});
            break;
...
FB.provide('', {
  ...
  init: function(options) {
    ...
    if (FB._nativeInterface) {
      FB._nativeInterface.init(FB._apiKey, function(e) {ToastL('¡Plugin de Facebook falló al iniciar!');});
    }
